...
public class DefaultUrlBuilder implements UrlBuilder {
    ...
    /**
     * Create an empty UrlBuilder, with no URL pattern. Using this constructor mandates that you MUST use the
     * build(String) form of build instead of the parameterless, build(), as the latter will throw IllegalStateException
     * in this state.
     */
    ...
    /**
     * Create a new UrlBuilder, passing in the URL pattern in which to substitute tokens. This URL pattern may represent
     * the entire URL or just the path portion (relative path).
     *
