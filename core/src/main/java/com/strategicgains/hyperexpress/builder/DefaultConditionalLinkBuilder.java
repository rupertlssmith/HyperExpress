...
/**
 * Extends LinkBuilder, adding a 'conditional' flag, where the value can either be "true" or a token (e.g. "{role}"). If
 * the flag is set to true and the resulting Link that is built contains a token (a URL token is not bound), then
 * build() returns null.
 *
